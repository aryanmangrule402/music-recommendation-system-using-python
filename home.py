...
def app():
    ...
    if st.session_state.username=='':
        ...
        ph='List about Favourite Music and artist you want'    
    post=st.text_area(label=' :orange[+ New Post]',placeholder=ph,height=None, max_chars=500)
    if st.button('List',use_container_width=20):
        if post!='':
    ...
    for doc in docs:
        d=doc.to_dict()
        try:
            st.text_area(label=':skyb[Posted by:] '+':orange[{}]'.format(d['Username']),value=d['Content'][-1],height=20)
        except: pass
